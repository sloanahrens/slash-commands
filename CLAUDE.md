...
2. **Pick a task** → `bd show <id>` for details, then `bd update <id> --status in_progress`
3. **Create new work** → `bd create "Description" --type task` (or `feature`, `bug`)
4. **Complete work** → `bd close <id>` when done
...
- Before task: `bd update <task-id> --status=in_progress`
- After task: `bd close <task-id>`
- Sync periodically: `bd sync`
- On completion: `bd close <feature-id>` then `bd sync`
...
3. **Do work** → `bd update <id> --status in_progress`, then `bd close <id>`
